...
export class ElrondHelper
    implements
        ...
        ChainEmitter<string, void, TransferEvent | UnfreezeEvent | ScCallEvent>
{
    ...
    async emittedEventHandler(
        event: TransferEvent | ScCallEvent | UnfreezeEvent
    ): Promise<void> {
        let tx: Transaction;
    ...
    private async eventDecoder(
        id: string
    ): Promise<TransferEvent | UnfreezeEvent | ScCallEvent | undefined> {
        await this.sender.sync(this.provider);
